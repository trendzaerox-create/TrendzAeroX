    ...
    <Link href={`/product/${product.id}`} className="block sm:h-full">
      <article className="group flex flex-col overflow-hidden rounded-[10px] border border-neutral-200 bg-white shadow-[0_2px_8px_rgba(0,0,0,0.10)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_10px_24px_rgba(0,0,0,0.16)] sm:h-full sm:rounded-[14px]">
        ...
        <div className="relative w-full overflow-hidden rounded-t-[10px] bg-white sm:rounded-t-[14px]">
          <div className="relative aspect-[4/3] w-full bg-white p-1 sm:p-2">
            <button
              type="button"
              onClick={handleWishlistClick}
            ...
            {firstImage ? (
              <div className="relative h-full w-full overflow-hidden rounded-[8px] bg-white sm:rounded-[10px]">
                <img
                  src={getImageUrl(firstImage)}
                  alt={product.title}
                  className="absolute inset-0 h-full w-full object-contain p-0.5 transition-all duration-500 group-hover:scale-[1.04] group-hover:opacity-0 sm:p-1"
                />
                ...
                {secondImage ? (
                  <img
                    src={getImageUrl(secondImage)}
                    alt={`${product.title} second view`}
                    className="absolute inset-0 h-full w-full object-contain p-0.5 opacity-0 transition-all duration-500 group-hover:scale-[1.04] group-hover:opacity-100 sm:p-1"
                  />
                ) : (
                  <img
                    src={getImageUrl(firstImage)}
              ...
              <div className="flex h-full w-full items-center justify-center rounded-[8px] bg-white text-xs font-medium text-neutral-500 sm:rounded-[10px]">
                No Image
              </div>
            )}
          </div>
        ...
        <div className="flex flex-col px-2 pb-2 pt-1 sm:flex-1 sm:px-2.5 sm:pb-2.5 sm:pt-1.5">
          <div className="flex items-center gap-1">
            <StarRating value={Number(avgRating)} size="11px" />
          ...
          <div className="mt-1.5 hidden items-center gap-1.5 sm:flex">
            ...
            <span className="line-clamp-1 text-[13px] font-semibold text-green-700">
              Offer Price ₹{offerPrice.toLocaleString("en-IN")}
            </span>
          </div>
        </div>
