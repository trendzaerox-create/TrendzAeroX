...
export default function OrderDetailsClient() {
  ...
  const [copiedTrackingId, setCopiedTrackingId] =
    useState(false);
  ...
  useEffect(() => {
    if (!token) {
      router.replace(
        `/login?next=/account/orders/${params.id}`
      );
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f5] px-4 py-8 md:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl">
        ...
        {successType === "paid" && (
          <div className="mb-6 rounded-2xl border border-green-200 bg-green-50 px-5 py-4 text-sm font-semibold text-green-700 shadow-sm">
            Payment successful. Your order has
            been confirmed.
          </div>
        )}
